...
设置里「双参谋 · 并行服务商」选第二家并填它的 Key 即开启：每轮把**完全相同**的请求并行发给两家，
各自给出完整意见（分析 + 3 个版本，单参谋时是 4 个以上），并排在同一条回复里，每段标注生成模型，对照着挑。
主参谋流式先出，并行参谋随后追加；并行侧失败只提示、不影响主意见。选「关闭」即回到单参谋模式。
两家的意见都会进上下文，追问时可以直接说「用 Gemini 那条改一下」。
