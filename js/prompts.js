...
/** 双参谋时两边共用的附加说明：任务完全相同，各给 3 个版本 */
export const DUAL_NOTE = `

【双参谋模式】
另一位参谋正在并行处理同一请求，用户会对照两份意见各取所长。因此上面「不少于 4 个版本」的要求改为：给 3 个版本，路数拉开；其余职责（局面分析、详略自适应、全部分寸规则）照旧，不需要提及或猜测另一位参谋的存在。`;
