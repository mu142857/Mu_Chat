...
/** 双参谋时给主参谋的附加说明：候选从 4 收到 2，分析照旧 */
export const DUAL_MAIN_NOTE = `

【双参谋模式】
本轮还有另一位参谋会并行补充 2 个候选版本。因此上面「不少于 4 个版本」的要求改为：你只给 2 个版本，路数拉开；分析部分照旧详略自适应。`;

/**
 * 第二参谋（补充候选）的系统提示：复用主提示词的全部人物/分寸/msg 规则，
 * 但砍掉分析职责，只出 2 条与常规写法气质错开的候选。
 */
export function buildAssistSystemPrompt(persona, memes = [], me = null) {
  return buildChatSystemPrompt(persona, memes, me) + `

【本次任务特化 · 你是第二参谋】
另一位参谋已负责局面分析和常规候选，你只做补充视角。以上格式规则里「分析」「不少于 4 个版本」等要求全部作废，改为：
- 不输出任何分析、说明、开场白、总结——一个字都不要。
- 只输出 2 个版本：每个版本先一行小标题点明路数（如「版本 · 直球」），随后紧跟 msg 块（要连发就连着写多个块）。
- 两个版本彼此路数拉开，并且刻意与最稳妥、最常规的写法错开——你的价值是提供另一种气质的选项，而不是重复别人大概率已经给过的答案。
- 其余所有规则（口吻、留话头、分寸、禁忌）照常遵守。`;
}
